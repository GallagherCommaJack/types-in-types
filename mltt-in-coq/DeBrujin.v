...
Lemma wk_beneath_wk : forall e d1 d2 n1 n2, d1 <= d2 -> 
   wk_deep n1 d1 (wk_deep n2 d2 e) = wk_deep n2 (d2 + n1) (wk_deep n1 d1 e).
  induction e; unfold wk_deep; simpl; intros;
  try (f_equal; reflexivity || apply IHe1 || apply IHe2 || apply IHe3 || apply IHe4; omega);
  fold_ops.
  - unfold_ops; destr_choices; f_equal; omega.
Qed.

Lemma wk_above_wk : forall e d1 d2 n1 n2, d1 >= n2 + d2 -> 
  wk_deep n1 d1 (wk_deep n2 d2 e) = wk_deep n2 d2 (wk_deep n1 (d1 - n2) e).
...
Hint Rewrite wk_beneath_wk wk_above_wk wk_deep_wk using omega.
Hint Rewrite wk_0_eq.
Hint Resolve wk_beneath_wk wk_above_wk wk_deep_wk wk_0_eq.
...
Lemma subst_lift_same : forall e v d n, wk_deep (S n) d e |> v // d = wk_deep n d e.
  ...
  - unfold wk_var,subst_var; destr_choices; f_equal; omega.
Qed.

Lemma subst_lift_gap : forall e v d1 d2 n, d2 <= d1 <= n + d2 -> wk_deep (S n) d2 e |> v // d1 = wk_deep n d2 e.
  unfold_ops; induction e; simpl in *; intros; destr_choices; f_equal; 
  try (apply IHe1 || apply IHe2 || apply IHe3 || apply IHe4); reflexivity || omega.
Qed.

Lemma subst_var_lift : forall i v d1 d2 n, d1 >= d2 -> 
  wk_deep n d2 (subst_var v d1 i) = subst_deep v (n + d1) (wk_var n d2 i).
  intros; unfold_ops; destr_choices; f_equal; fold_ops; autorewrite with core; reflexivity || omega.
Qed.

Hint Rewrite subst_lift_same.
Hint Rewrite subst_lift_gap subst_var_lift using omega.
Hint Resolve subst_lift_same subst_lift_gap subst_var_lift.

Lemma lift_below_subst : forall e v d1 d2 n, d1 <= d2 -> wk_deep n d1 (e |> v // d2) = (wk_deep n d1 e) |> v // (n + d2).
  unfold subst_deep,wk_deep; induction e; intros; simpl; fold_ops; f_equal; try rewrite plus_n_Sm;
  try (reflexivity || apply subst_var_lift || apply IHe1 || apply IHe2 || apply IHe3 || apply IHe4; omega).
Qed.

Lemma lift_above_subst : forall e v d1 d2 n, d1 > d2 -> 
  wk_deep n d1 (e |> v // d2) = wk_deep n (S d1) e |> wk_deep n (d1 - d2) v // d2.
  unfold_ops; induction e; intros; simpl; fold_ops; f_equal; 
  try (reflexivity || apply IHe1 || apply IHe2 || apply IHe3 || apply IHe4; omega).
  - unfold_ops; destr_choices; fold_ops; try apply wk_above_wk; f_equal; omega.
Qed.
